...
const renderCurrencyChange = (change: number) => {
  if (change === 0) {
...
export function AdminDashboard({ currentUser, initialCurrencies, initialUsers }: AdminDashboardProps) {
  const [currencies, setCurrencies] = useState(initialCurrencies);
  const [users, setUsers] = useState(initialUsers);
  const [draftCurrencies, setDraftCurrencies] = useState(initialCurrencies);
  ...
  const handleSaveCurrencies = async () => {
    const changes = draftCurrencies.filter((draft) => {
      const original = currencies.find((c) => c.id === draft.id);
  ...
  const handleAddCurrency = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if ([buyPrice, sellPrice].some((v) => Number.isNaN(v) || v <= 0)) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const [updatedCurrencies, updatedUsers] = await Promise.all([
        getCurrencies(),
        getUsers(),
      ]);
  ...
  const handleSeedCurrencies = async () => {
    startTransition(async () => {
      try {
        const result = await seedDefaultCurrencies();
        const updated = await getCurrencies();
        setCurrencies(updated);
        toast.success(result.message);
      } catch (error) {
        console.error(error);
        toast.error("تعذر إضافة العملات الافتراضية");
      }
    });
  };
  ...
  const handleAddAdminAccount = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      try {
        ...
        const message = error instanceof Error ? error.message : "تعذر إضافة المشرف";
        toast.error(message);
      }
    });
  };
  ...
  const handleRemoveAdminAccount = (id: string) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateUserRole(id, "USER");
        const updatedUsers = await getUsers();
        setUsers(updatedUsers);
        ...
        const message = error instanceof Error ? error.message : "تعذر إزالة صلاحيات المشرف";
        toast.error(message);
      }
    });
  };
        ...
        <Card className="shadow-medium">
          <CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex gap-2 flex-wrap">
              {currencies.length === 0 && (
                <Button variant="outline" onClick={handleSeedCurrencies} disabled={isPending}>
              ...
              <Button onClick={handleSaveCurrencies} disabled={isSaving} className="gap-2">
                {isSaving ? "جارٍ الحفظ..." : <><Save className="w-4 h-4" /> حفظ الكل</>}
              </Button>
            </div>
          </CardHeader>
                ...
                {draftCurrencies.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
                      لا توجد عملات. أضف عملة جديدة أدناه أو استخدم زر &quot;إضافة عملات افتراضية&quot;.
                    </TableCell>
                  </TableRow>
                ) : (
                  draftCurrencies.map((currency) => (
                    <TableRow key={currency.id}>
                      <TableCell>
                        <Input
                          value={currency.name}
                          onChange={(e) => handleDraftChange(currency.id, "name", e.target.value)}
                        />
                      </TableCell>
                      <TableCell>
                        <Input
                          value={currency.code}
                          onChange={(e) => handleDraftChange(currency.id, "code", e.target.value.toUpperCase())}
                      ...
                      <TableCell>
                        <Input
                          ...
                          onChange={(e) => handleDraftChange(currency.id, "buyPrice", e.target.value)}
                      ...
                      <TableCell>
                        <Input
                          ...
                          onChange={(e) => handleDraftChange(currency.id, "sellPrice", e.target.value)}
                          className="text-center"
                        />
                      </TableCell>
                      ...
                      <TableCell className="text-center">{renderCurrencyChange(currency.change)}</TableCell>
            ...
            <form className="grid gap-4 md:grid-cols-2" onSubmit={handleAddCurrency}>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNewCurrency((prev) => ({ ...prev, buyPrice: e.target.value }))}
                  required
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNewCurrency((prev) => ({ ...prev, sellPrice: e.target.value }))}
                  required
                />
              </div>
          ...
          <CardContent className="space-y-6">
                      ...
                      <TableRow key={admin.id}>
                        ...
                        <TableCell className="text-center">
                          <Button
                            variant="ghost"
                            size="icon"
                            disabled={adminAccounts.length <= 1 || isPending}
            ...
            <form className="grid gap-4 md:grid-cols-4" onSubmit={handleAddAdminAccount}>
              ...
              <div className="md:col-span-4 flex justify-end">
                <Button type="submit" className="gap-2" disabled={isPending}>
                  {isPending ? "جاري الإضافة..." : <><Plus className="w-4 h-4" /> إضافة مشرف</>}
                </Button>
              </div>
            </form>
          </CardContent>
